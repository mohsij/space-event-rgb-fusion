...
from core.function import train_posehrnetfusion_nofusion_rgbonly
...
from utils.transforms import EventNormalise, FillEventBlack, RandomEventNoise, RandomEventPatchNoise, RandomUniformNoise, RandomColourNoise, RandomBloomV2, RandomBlur
...
def main():
    ...
    train_dataset_event_rgb = eval('dataset.'+cfg.DATASET.DATASET)(
        ...
        transforms_rgb=transforms.Compose([
            ...
            transforms.ToTensor(),
            transforms.ColorJitter(brightness=0.75, contrast=0.8, hue=0.4, saturation=0.25),
            transforms.RandomApply([RandomUniformNoise()], p=0.5),
            transforms.RandomApply([RandomColourNoise()], p=0.5),
            transforms.RandomApply([RandomBlur()], p=0.2),
            # normalize_rgb,
        ]),
    )
    ...
    for epoch in range(begin_epoch, cfg.TRAIN.END_EPOCH):
        ...
        # train for one epoch
        train_posehrnetfusion_nofusion_rgbonly(cfg, 
            train_loader_event_rgb, model_rgb, 
            model_hrnet_encoder_rgb, heatmap_loss_rgb,
            optimizer_rgb, epoch,
