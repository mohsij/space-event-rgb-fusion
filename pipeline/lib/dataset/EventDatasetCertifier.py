...
class EventDatasetCertifier(Dataset):
    def __init__(self, cfg, root, image_set, is_train, frames_dir, transforms_event=None):
        self.num_joints = 0
        self.pixel_std = 200
        ...
        with open(cfg.DATASET.DENSE_CAD_POINTS, 'r') as cad_file:
            cad_model = torch.tensor(np.array(json.load(cad_file)['dense_points']), device=device, dtype=torch.float)
        self.certifier = Certifier(cfg, self.intrinsics_tensor_event, self.landmarks_tensor, cad_model, cfg.TRAIN.BATCH_SIZE_PER_GPU_ADVERSARIAL_SET)
